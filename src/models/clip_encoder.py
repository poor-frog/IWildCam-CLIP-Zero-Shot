...
class CLIPEncoder(torch.nn.Module):
    def __init__(self, args, keep_lang=False):
        ...
        if args.model == 'ViT-L-14':
            ...
            self.model, preprocess = clip.load(
...
class ClassificationHead(torch.nn.Linear):
    ...
    def forward(self, inputs):
        # Align dtypes for MPS compatibility (MPS doesn't support mixed-precision matmul)
        if inputs.dtype != self.weight.dtype:
            inputs = inputs.to(dtype=self.weight.dtype)
        if self.normalize:
...
class ImageEncoder(torch.nn.Module):
    def __init__(self, args, keep_lang=False):
        ...
        self.model, preprocess = clip.load(args.model, args.device, jit=False)
        self.train_preprocess = preprocess
        self.val_preprocess = preprocess
