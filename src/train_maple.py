...
from src.models.maple import (
    CustomMaPLeCLIP,
    ensure_openai_vit_b32_for_maple,
    eval_maple_single_dataset,
    get_maple_prompt_learner,
    load_maple_prompt_learner,
    save_maple_prompt_learner,
    train_maple_one_epoch,
)
...
def resolve_save_path(save_path):
    ...
    if os.path.isdir(save_path) or not os.path.splitext(save_path)[1]:
        return os.path.join(save_path, "maple_prompt_learner.pt")
    return save_path
...
def resolve_best_checkpoint_path(args):
    ...
    if args.save is not None:
        ...
        if os.path.isdir(args.save) or save_path.suffix == "":
            return str(save_path / "maple_prompt_learner_best.pt")
        return str(save_path.with_name(f"{save_path.stem}_best{save_path.suffix}"))
    return os.path.join("checkpoints", "maple_prompt_learner_best.pt")
...
def main(args):
    ...
    ensure_openai_vit_b32_for_maple(clip_encoder.model, args.model)
    ...
    model = CustomMaPLeCLIP(args, classnames, clip_encoder.model).to(args.device)
    model = maybe_data_parallel(model, args)
    if hasattr(model, "module"):
        print(f"Using DataParallel on {torch.cuda.device_count()} CUDA devices")

    if args.load is not None:
        load_maple_prompt_learner(model, args.load, args.device)

    optimizer = torch.optim.AdamW(get_maple_prompt_learner(model).parameters(), lr=args.lr, weight_decay=args.wd)
    ...
    for epoch in range(1, args.epochs + 1):
        stats = train_maple_one_epoch(model, train_data.train_loader, optimizer, args, epoch, wandb=wandb)
        scheduler.step()
        ...
        if val_dataset is not None:
            print(f"Validating MaPLe on {args.val_dataset} at epoch {epoch}...")
            val_results = eval_maple_single_dataset(model, val_dataset, args)
            ...
            if best_score is None or val_score > best_score:
                ...
                best_epoch = epoch
                save_maple_prompt_learner(model, best_checkpoint_path, args, classnames)
                print(
                    f"Saved best MaPLe prompt learner to {best_checkpoint_path} "
    ...
    if args.save is not None:
        ...
        save_maple_prompt_learner(model, save_path, args, classnames)
        print(f"Saved MaPLe prompt learner to {save_path}")

    if args.eval_datasets is not None and best_epoch is not None and not args.no_load_best_for_eval:
        load_maple_prompt_learner(model, best_checkpoint_path, args.device)
        print(
            f"Loaded best MaPLe prompt learner from {best_checkpoint_path} "
    ...
    if args.eval_datasets is not None:
        for dataset_name in args.eval_datasets:
            print(f"Evaluating MaPLe on {dataset_name}...")
            eval_dataset = build_eval_dataset(dataset_name, clip_encoder, args)
            results = eval_maple_single_dataset(model, eval_dataset, args)
