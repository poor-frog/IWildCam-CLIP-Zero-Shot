...
from src.models.logit_adjustment import apply_class_bias, apply_logit_adjustment
...
def eval_coop_single_dataset(model, dataset, args, desc="CoOp eval", tau=None, class_priors=None, class_bias=None):
    model.eval()
    ...
    with torch.no_grad():
        for batch_index, data in enumerate(tqdm(loader, desc=desc)):
            ...
            logits = apply_class_bias(logits, class_bias)
